...
SPISettings ls7366SPI(2000000, MSBFIRST, SPI_MODE0);
...
void LS7366::clear_mode_register_0() {
  ...
  SPI.beginTransaction(ls7366SPI);
...
long LS7366::left_extend_MSB(long val) {
  ...
  long MSB;
  if (datawidth == 4) {
    value_to_return = val;  // Nothing to do in 4 byte mode
  } else {
    MSB = (val >> (datawidth*8 - 1)) & 0x0001;
    if (MSB == 0) {
      value_to_return = val;  // If the MSB is 0, there is nothing to extend.
    } else {
      switch (datawidth) {
        case 1: { 
